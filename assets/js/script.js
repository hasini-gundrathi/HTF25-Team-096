...
function showPrintableSeatingChart() {
  ...
  let html = `<h1 style="text-align:center;">Exam Seating Chart (Room Wise)</h1>`;

  rooms.forEach(room => {
    // Layout for this room
    const rows = parseInt(room.rows);        // Number of rows (from room object, e.g. 10)
    const cols = parseInt(room.seatsPerRow); // Number of columns (from room object, e.g. 6)
    // Empty grid
    let grid = Array.from({ length: rows }, () => Array(cols).fill(null));

    // Place allocations in the grid using seat label
    allocations
      .filter(a => a.roomId === room.roomId)
      .forEach(a => {
        // Parse seat label (e.g., "A1") into indices
        const rowIndex = a.seatNo.charCodeAt(0) - 65; // 'A' => 0
        const colIndex = parseInt(a.seatNo.slice(1), 10) - 1; // '1' => 0
    ...
    html += `<h2 style="margin-top:30px;">${room.name} (${room.roomId})</h2>
      <table border="1" cellpadding="10" cellspacing="0" style="border-collapse:collapse; margin:auto; margin-bottom:35px; min-width:50vw;">
      <thead>
        <tr><th></th>`;
    // Column headers for benches
    for (let c = 0; c < cols; c++) {
      html += `<th>Bench ${c + 1}</th>`;
    }
    html += `</tr>
      </thead>
      <tbody>`;
    ...
    for (let r = 0; r < rows; r++) {
      ...
      for (let c = 0; c < cols; c++) {
        ...
        html += `<td style="text-align:center; background:#e6f4f3; min-width:140px; height:70px;">
          ${alloc 
            ? `<div style="font-weight:600; font-size:1.15em; color:#237e91;">${alloc.seatNo}</div>
